...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($azione === 'restituisci') {
        ...
        if (!$p) {
            ...
            $stmt = $conn->prepare(
                "UPDATE prestiti SET data_restituzione = CURDATE(), multa = ? WHERE id = ?"
            );
...
$prestiti_aperti = $conn->query(
    "SELECT p.id, p.data_inizio, p.data_scadenza, p.rinnovi,
            l.titolo, l.autore,
            u.nome AS nome_lettore, u.email
     FROM prestiti p
